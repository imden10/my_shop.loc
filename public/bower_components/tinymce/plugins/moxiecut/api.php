...
$uploaddir = "/public/uploads/tiny"; //путь на сервере
//$uploaddir = "/uploads/tiny"; //путь на локалке
$globaldirs = array("images"=>array("ext"=>"jpg,gif,png,bmp"),
//                    "documents"=>array("ext"=>"doc,xls,pdf,ppt,txt,csv,docx,xlsx"),
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if($_GET["action"] == "upload") //закачка файлов
    {
        ...
        $file_extension = substr(strrchr($name,"."),0); //вырезаем расширение файла с точкой
        $file_name = str_replace($file_extension, '', $name); // вырезаем только название файла
        $s = (string) $file_name; // преобразуем в строковое значение
        ...
        $file_name = $s;
        $name = $file_name.$file_extension; //соединяем имя и расширение файла
